<template>
	<view class="bg-white" @click.stop="openDetail">
		...
		<view class="d-flex a-center p-2 border-bottom border-light-secondary">
			<text class="text-muted font-md">{{item.create_time}}</text>
			<text class="main-text-color ml-auto font-md">{{item.status}}</text>
		</view>
		<!-- 主体 -->
		<view class="px-2">
			<block 
				v-for="(order, orderIndex) in item.order_items"
				:key="orderIndex"
			>
		...
		<view class="d-flex j-end a-center px-2 pb-2">
			<view 
				class="rounded border border-light-secondary py-1 px-2 text-secondary" 
				hover-class="bg-light-secondary"
				@click.stop="openLogistics"
			>
				查看物流
			</view>
			<view class="ml-2 rounded border border-light-secondary py-1 px-2 text-secondary" hover-class="bg-light-secondary">
				确认收货
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			openDetail() {
				uni.navigateTo({url: '/pages/order-detail/order-detail'})
			},
			openLogistics() {
				uni.navigateTo({
